...
(function (global) {
  ...
  var CONFIG = {
     ...
     * @brief 風の線全体の濃さの倍率。
     *
     * 一本一本がはっきり見える必要はない。風は「そこにある」と分かれば
     * 十分で、濃いと画面の主役を奪ってしまう。段階が上がったことは、
     * 本数と画面全体の明るさのほうが受け持つ。
     *
     * 濃さの散らばりは別に掛けてある。ここは全体をまとめて上げ下げする
     * ための1か所で、薄さの調整はここだけを触れば済む。
     */
    windAlpha: 0.55,
   ...
   * 進み具合（windPhase）を進めるのは呼ぶ側の仕事。描くことと時間を
   * 進めることを分けておくと、描く回数が変わっても速さが変わらない。
   *
  ...
  function drawWindLines(step, kick) {
    ...
    for (var i = 0; i < count; i++) {
       ...
       */
      var alpha = CONFIG.windAlpha *
                  (0.10 + level * 0.62) * M.clamp(near * 3, 0, 1) *
                  (0.8 + kick * 0.45) *
       ...
       * 1本を1色で引く。
       *
       * 根元から先端へ濃さを変えることもできるが、途中で段になって
       * 色が変わったように見え、なめらかにするにはグラデーションを
       * 作るしかない。それは本数ぶん作り直すことになって重い。
       *
       * 1色でも、本ごとに濃さを散らしてあるので、薄い筋と濃い筋が
       * 重なって流れる。向きは、手前ほど長く速いことで伝わる。
       *
       * 描くのはここではなく、濃さと太さの近いものをまとめる入れ物へ。
       */
      addWindSegment(alpha, width, tx, ty, x, y);
    }
  ...
  function frame(ms) {
    ...
    // 風の線を進めるのはここ。描く側では進めない。描く回数が変わっても
    // 流れる速さが変わらないようにするため。
     ...
     * ゲートなので、これを外すと光そのものが弱くなり、背景だけが
     * ぼんやり光る絵になってしまった。下描きにあたるので、この後の
     * 加算で振り切れても構わない。同じものを光の後にもう一度描き、
     * そちらが最後に残る。
     *
     * 風の線はここでは描かない。数が多く、2度描くと1枚あたりの手間が
     * そのまま倍になる。にじみの元はほとんどゲートなので、外しても
     * 光の強さはほぼ変わらない。
     */
    if (glareAmount > 0.01 && scene.front) {
      applyZoom(zoom, sx, sy);
      scene.front(f);
    }
     ...
     * ゲートは光の素としても上で描いてあるので、にじみ（ハロー）が
     * 周りに残る。風の線はここだけで、光の下をくぐらせない。
     *
