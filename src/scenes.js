...
(function (global) {
  ...
  function drawHall(f, travel) {
    ...
    // 描画が追いついていない環境でも、こちらは止めない。
    //
    // 自前の塗りは素の JavaScript なので、ブラウザによる速度差がほとんど
    // ない。Canvas のグラデーションが遅い環境ではむしろこちらが軽く、
    // 逃げ道として機能する。負荷はバッファの解像度の方で調整する。
    var rb = (smoothOn && f.rasterBuf) ? f.rasterBuf : null;
  ...
  /**
   * @brief 前景（リングと自機）。1枚のあいだに2度呼ばれる（光の素として1度）。
   *
   * 1度目は光の素として、2度目は光を乗せた後に。最後に残るのは2度目
   * なので、色は塗ったとおりになり、周りには1度目から生まれたにじみが
   * 残る。リングは彩度を上げて塗っているため、光の下に置いたままだと
   * 加算で振り切れ、色相が動いて切れ目の位置が読み取りにくかった。
   *
   * 背景と立方体は光の前に一度描くだけでよい。そちらはにじんで正しい。
