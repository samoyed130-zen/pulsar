...
感度・音・背景・塗り方・開放済みステージの選択は端末に保存され、次回も維持されます。音は既定で入っていますが、ブラウザの自動再生制限があるため、実際に鳴り始めるのは最初に画面へ触れたときです。
...
当たり判定・3D 計算・タイムライン・難易度の条件・一時停止の挙動などを 233 件のテストで検証しています。テストは実際に次のバグを検出しました。
