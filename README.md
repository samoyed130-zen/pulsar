...
ここは物理ではなく勢いを見せる絵なので、**今の位置を中心へ向かって縮めた点**を尾の根元にしています。奥でも手前でも同じ細長さに見え、進むほど実際の長さは伸び、時間もフレームレートも関わりません。濃さは根元から先端へ**1本の中で上げて**います。最初は先端に濃い線を重ねていたのですが、色の違う短い棒が乗っているようにしか見えませんでした。次にグラデーションで滑らかに変えたところ、今度は線ごとにグラデーションを作り直すことになり、目に見えて遅くなりました。いまは2つの区間に区切って濃さだけを変えています。それでもコンボが最大の段階では 60 回/秒に届きませんでした。塗る面積は小さいのに、**1本ずつ線を引く命令を呼ぶ手間**（最大 110 回）が積み上がっていたためです。濃さと太さを段に丸め、同じ段のものを一つの道筋（`Path2D`）へまとめてから引くようにして、呼び出しを大きく減らしました。丸めのずれは濃さで 0.07、太さで 0.5px までに収めてあるので、絵は変わりません。
