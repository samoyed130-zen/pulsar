...
**3. グレアを止めた分は、描く側の明るさで補う。**
グレアは光を加算する処理なので、止めるとその分だけ画面全体が沈みます。そこで、グレアを使わない環境では立体を塗るときの明るさを 1.35 倍にしています。加算で後から足すか、最初から明るく塗るかの違いで、同じ「光っている」印象へ近づけています。
...
当たり判定・3D 計算・タイムライン・難易度の条件・一時停止の挙動などを 218 件のテストで検証しています。テストは実際に次のバグを検出しました。
