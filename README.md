...
| 触れると操縦できる区間 | 場面のひとつ（METABALLS） |
| --- | --- |
| ![操縦中の画面](assets/screenshot2.jpg) | ![メタボールの場面](assets/screenshot3.jpg) |
...
- **一本指で動かす** : 動かした量だけ自機が回ります。画面のどこを触っても構いません。上下左右のどちらでも、動かした通りに回ります。
...
同じ「画面を指す」操作でも、指とマウスでは事情が違います。マウスは画面の外から指すので、円周上の位置をそのまま狙いにできます。指は**自分の手が画面を覆う**ので、同じ作りにすると狙いたい場所を自分で隠すことになります。とくに縦長の画面では、輪の下半分が手の下に入ります。

そこで指の端末だけ、**一本指を動かした量**で回すようにしました。画面のどこを触っていても構いません。

向きは上下左右のどれでも構いません。自機は輪の上を回るので、**動かすべき向きは自機のいる場所で変わります**。輪の上にいるときは左右、右にいるときは上下が「進む向き」です。指の動きを、自機のいる場所での**輪の接線**へ落とし込めば、どちらへ動かしても動かした通りに回ります。倍率は 1 にしてあり、指が進んだ長さと自機が輪の上を進む長さが一致します。指の下を自機が滑っていく感じになり、狙いをどれだけ動かせばよいかが読めます。

同じ理由で、キーボードも **↑ ↓** を受けるようにしました。押した向きへ自機が回り込みます。輪の真上と真下だけは上下の向きが接線と直角に交わって効かなくなるので、そこにも下限を設けて必ずどちらかへ回します。動かないのは壊れて見えるからです。

副産物として、**触れた瞬間に自機が飛ばなくなりました**。位置ではなく動いた量で操るので、指を置いただけでは何も起きません。触れた場所へ吸い寄せられることも、指を止めたのに回り続けることもありません。

追いかけるのは**最初に触れた一本だけ**です。2本目のぶんまで足すと、持ち替えただけで離れた場所どうしの差が一度に入り、自機が飛びます。
...
設定はすべて端末に保存され、次回も維持されます（操作感度・ガイドサークル・背景・塗り方・音・疑似グレア固定・FPS表示・開放済みステージ）。音は既定で入っていますが、ブラウザの自動再生制限があるため、実際に鳴り始めるのは**最初に何か操作したとき**です。

この「何か」を取りこぼしていた時期があります。音を起こす処理をキャンバスにだけ付けていたため、タイトル画面のボタンを押しても届かず、iPhone では最初のステージに入るまで無音のままでした。タイトルもメニューも画面を覆う別の層なので、そこを押してもキャンバスには沈んできません。いちばん最初に押すのはたいていタイトルのボタンなので、**いちばん肝心な操作を取りこぼしていた**ことになります。いまは窓のいちばん外側で、押す・触れる・キーを打つのすべてを受けています。

受け口を広げてもなお、タイトルでは鳴りませんでした。原因は別にあり、**操作の中で作った音声が、止まった状態で生まれてくる**ことでした。作った場でそのまま起こし、長さ1つぶんの無音を一度鳴らして口を開けています。

音声を**作る**経路と、**起き続けさせる**経路も分けました。iPhone では操作の外で作った音声は止まったままになるので、毎フレーム呼ぶ側（`keepAlive`）では作らせません。作ってよいのは、操作を起点に呼ぶ側（`wake`）だけです。
...
カメラも**進む向き**を向きます。投影そのものは真正面しか知らないので、通路の接線を求め、世界の側をその逆に回してから投影しています。曲がるたびに消失点が動き、通路にパースの傾きが出ます。向けたぶん消失点は画面の中央から外れるので、そのずれをリング・自機・浮かぶ立体にも渡して重ねています（手前は6割に抑えています。丸ごと動かすと狙う的まで画面の端へ寄るためです）。
...
ゲートを**光の素にも入れて**（1枚のあいだに2度描いて）、色は保ったままハローだけ出す手も試しました。理屈は通るのですが、にじみが戻れば切れ目の縁もまた滲みます。読み取るためのものを滲ませては元も子もないので、捨てました。2度描くぶん重くもなります。

いま光の素になるのは**背景と立方体だけ**です。それらは今までどおり光の前に一度描いています。**眩しさは景色の役目、判断の材料はくっきり**、という切り分けになりました。
...
assets/
  ...
  screenshot.jpg  この README に載せている画面（タイトル）
  screenshot2.jpg 同上（操縦中）
  screenshot3.jpg 同上（METABALLS）
src/
...
- **スマートフォンでは縦持ちを推奨します。** 片手で持ったまま、親指を左右に
  滑らせるだけで操作できます
...
ブラウザは、利用者が一度も操作していないページから音を出すことを禁じています。スマートフォンには例外がなく、こちらで作ったタップ（`dispatchEvent`）は「利用者の操作」として数えられないため、回避する手立てはありません。タイトル画面はどこを押しても音が起きるようにしてあり、鳴っていない間は「♪ 画面をタップすると音が鳴ります」と表示して、何をすれば鳴るのかが分かるようにしています。
