...
ただし単に止めると、光を加算する処理が丸ごと消えるため画面が沈み、暗く見えます。そこで **`filter` を一切使わない加算ライト** を代わりに重ねています。画面を 1/12 ほどに思い切り縮め、それを少しはみ出す大きさへ拡大して加算で戻すだけです。縮小のときの平均化と拡大の補間がぼかしの役目を果たし、はみ出した分だけ光が輪郭の外へ漏れます。暗部を切り落とせないので眩しさの誇張は本来のグレアより弱まりますが、画面の明るさは取り戻せます。加算する強さを本来より弱めているのは、暗い部分まで一緒に持ち上がって白く濁るのを避けるためです。

**2. それ以外の環境では、実際にかかった時間を測って落とす。**
1フレームが 22ms を超え続けたら、建造物の見通す区画数と分割数を減らし、面の塗りを**三角形を割って塗り分ける方式**に切り替えます。これはグラデーションの生成そのものが重い場合への対処で、三角形を重心から3つに割り、それぞれを「その辺の2頂点と重心の平均の明るさ」で塗ります。厳密な補間ではありませんが、塗りつぶし自体は速く、単色に潰すのと違って面の表情が残ります。

**3. 塗る側の明るさでも補う。**
加算ライトは後から光を足す処理なので、元の絵が暗いと足す光もありません。グレアを本来の形で使えない環境では、立体を塗るときの明るさそのものを 1.35 倍にして下地を持ち上げています。

**一度落としたら元に戻しません。** 上と同じ往復を避けるためです。ちらつくよりは、簡素なまま滑らかに動く方が快適だと判断しました。Chrome・Edge・Safari のように余裕がある環境では、グラデーションのまま動きます。
...
当たり判定・3D 計算・タイムライン・難易度の条件・一時停止の挙動などを 221 件のテストで検証しています。テストは実際に次のバグを検出しました。
...
- Firefox でも動作しますが、Canvas のぼかしが遅いため、自動的に軽い描画へ切り替わります（見た目は少し簡素になります）
