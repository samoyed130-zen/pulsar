...
段階が上がると、**画面全体の輝度も上がります**。風の線は画面の端を流れるので、中心のリングを見ている目には届きにくいためです。グレア（光のにじみ）の強さを段階に比例させていて、最大の段階では 0.41 → 0.60（拍の頂点では 0.66 → 0.96）まで増えます。加算合成の上限を超えないところで止めてあります。
