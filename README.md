...
**スマートフォンでは、ページを開いただけでは音が鳴りません。画面に一度触れると鳴り始めます。**（パソコンでは開いた時点から鳴ります。）

ブラウザは、利用者が一度も操作していないページから音を出すことを禁じています。パソコンでは何度も訪れたページに例外が認められますが、スマートフォンにはその例外がありません。こちらで作ったタップ（`dispatchEvent`）は操作として数えられないため、回避する手立てもありません。タイトルはどこを押しても音が起きるようにしてあり、鳴っていない間は「♪ 画面をタップすると音が鳴ります」と出して、何をすれば鳴るのかが分かるようにしています。
