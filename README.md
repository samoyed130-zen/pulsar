...
事前に見分けることはできないので、**実際にかかった時間を測って落とす**ようにしました。1フレームが 22ms を超え続けたら次の3つを切り替えます。

1. グレアを止める（最も重い処理）
2. 建造物の見通す区画数と、壁や床の分割数を減らす
3. 面の塗りを、グラデーションから**三角形を割って塗り分ける方式**に変える

3 は、グラデーションの生成そのものが重いための代替です。三角形を重心から3つに割り、それぞれを「その辺の2頂点と重心の平均の明るさ」で塗ります。厳密な補間ではありませんが、塗りつぶし自体は速いので面の数が増えても耐えますし、単色に潰すのと違って面の表情が残ります。

13ms を下回り続けたら元に戻します。基準を離してあるので、行ったり来たりはしません。Chrome・Edge・Safari のように余裕がある環境では、グラデーションのまま動きます。
...
当たり判定・3D 計算・タイムライン・難易度の条件・一時停止の挙動などを 214 件のテストで検証しています。テストは実際に次のバグを検出しました。
