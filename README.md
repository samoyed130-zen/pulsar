...
なお、**塗る側の明るさを掛けて持ち上げる方法は採っていません。** 明るさには上限があるため、多くの面が上限で頭打ちになり、隣り合う面の差が消えて境目だけが段差として残ってしまうためです。暗さは、あくまで光を足す側で取り戻しています。
...
当たり判定・3D 計算・タイムライン・難易度の条件・一時停止の挙動などを 219 件のテストで検証しています。テストは実際に次のバグを検出しました。
