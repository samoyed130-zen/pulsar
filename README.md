...
ただし順番を変えただけでは、**光そのものが弱くなりました**。グレアは画面の明るいところを集めて作るので、いちばん明るいゲートと風の線を外すと、背景だけがぼんやり光る絵になります。そこで前景は**1枚のあいだに2度描いています**。1度目は光の素として（ここで振り切れても構いません、下描きなので）、2度目は光を乗せた後に。最後に残るのは2度目なので色は塗ったとおりで、周りには1度目から生まれたにじみ（ハロー）が残ります。

2度描くぶんの手間は、ゲートが十数本の弧、風の線が 19 回の呼び出しなので、60 回/秒を保てる範囲に収まっています。背景と立方体は今までどおり光の前に一度描くだけです。**眩しさは景色の役目、判断の材料はくっきり**、という切り分けになりました。
