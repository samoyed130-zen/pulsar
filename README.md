...
描き方は**星空の場面と同じ**です。線1本ずつに奥行きを持たせ、それを毎フレーム手前へ進め、**前の位置から今の位置までを結びます**。つまり線の長さは「この1枚のあいだに動いた距離」そのものです。

最初は半径と長さを式で決めていましたが、放射状の棒が伸び縮みしているようにしか見えませんでした。透視投影（焦点距離 ÷ 奥行き）に任せると、手前ほど速く大きく開いていく加速が計算するまでもなく出てきます。実測では、奥にいるあいだ 1 枚 10px ほどだった筋が、目の前を抜ける頃には 490px まで伸びます。消える奥行きを 0 にできない（投影が発散する）ので手前で打ち切っていますが、そこはすでに画面の外なので、**溶けて消えるのではなく飛び抜けて**いなくなります。
