...
縮小後は数十 px 四方しかないため、9回重ねても負荷はごくわずかです。ぼかしの質は劣りますが、光が漏れて輝度差が誇張されるという肝心の部分は残ります。
...
当たり判定・3D 計算・タイムライン・難易度の条件・一時停止の挙動などを 222 件のテストで検証しています。テストは実際に次のバグを検出しました。
