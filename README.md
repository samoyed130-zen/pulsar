...
**持ち上げるのは彩度だけにしています。** 明るさを掛けて補うと、上限で頭打ちになる面が増え、隣り合う面の差が消えて境目だけが段差として浮いてしまいます。一方、彩度は面の向きによる差を作っていないので、同じように掛けても境目は生まれません。乗算による暗部潰しでは色も一緒に沈み、全体が灰色に寄るため、そこだけを 1.35 倍で戻しています。明るさは、あくまで光を足す側で取り戻します。
...
当たり判定・3D 計算・タイムライン・難易度の条件・一時停止の挙動などを 220 件のテストで検証しています。テストは実際に次のバグを検出しました。
